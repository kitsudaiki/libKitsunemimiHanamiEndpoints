/**
 * @file        endpoint_parser_interface.cpp
 *
...
Kitsunemimi::Hanami::EndpointParserInterface* EndpointParserInterface::m_instance = nullptr;
 ...
 */
EndpointParserInterface::EndpointParserInterface(const bool traceParsing)
{
...
EndpointParserInterface::getInstance()
{
    if(m_instance == nullptr) {
        m_instance = new EndpointParserInterface();
    }
 ...
 * @brief destructor
 */
EndpointParserInterface::~EndpointParserInterface() {}
                               ...
                               ErrorContainer &error)
{
    ...
    Kitsunemimi::Hanami::EndpointParser parser(*this);
 ...
 */
const std::string
EndpointParserInterface::removeQuotes(const std::string &input)
{
    // precheck
    if(input.length() == 0) {
        return input;
 ...
 */
void
EndpointParserInterface::error(const Kitsunemimi::Hanami::location& location,
                             const std::string& message)
{
    ...
    // build error-message
    m_errorMessage =  "ERROR while parsing endpoint-formated string \n";
